...
class RemainingDataFetcher {
  constructor() {
    ...
    // 残りの年度（2025, 2030は取得済み）
    this.remainingYears = [2035, 2040, 2045, 2050];
  }
  ...
  async fetchSingleYear(year) {
    ...
    for (const prefCode of Object.keys(this.prefectureCodes)) {
      try {
        ...
        console.log(`✅ ${this.prefectureCodes[prefCode].name} completed (${completedPrefectures}/${totalPrefectures})`);
  ...
  async fetchRemainingData() {
    console.log('🚀 Fetching remaining years data...\n');
    
    for (const year of this.remainingYears) {
    ...
    console.log('🎉 All remaining data fetching completed!');
...
async function main() {
  try {
    const fetcher = new RemainingDataFetcher();
    await fetcher.fetchRemainingData();
  } catch (error) {
