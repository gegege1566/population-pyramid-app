...
class CoopMemberProjectionGenerator {
  constructor() {
  ...
  loadPopulationData(year) {
    const filename = `population_national_${year}.json`;
    const filepath = path.join(this.populationDir, filename);
    
    if (!fs.existsSync(filepath)) {
      throw new Error(`Population data not found: ${filepath}`);
    }
  ...
  calculateBaseShareRates() {
    const basePopulation = this.loadPopulationData(2025);
    const shareRates = new Map();

    // 全国のシェア率を計算
    const prefectureCode = '00000';
    ...
    this.baseMemberData.ageComposition.forEach(comp => {
      ...
      if (maleData && femaleData) {
        // 全国データは実人数なので千人単位に変換
        const totalPopulation = (maleData.population + femaleData.population) / 1000;
  ...
  // 減衰率を計算
  calculateDecayRate(fromAgeGroup, toAgeGroup, fromYear, toYear) {
    const fromPopulation = this.loadPopulationData(fromYear);
    const toPopulation = this.loadPopulationData(toYear);
    
    // 全国データから減衰率を計算
    const fromMale = fromPopulation.find(p => 
      p.prefectureCode === '00000' && 
  ...
  // 将来年度の組合員数を推計
  generateProjection(targetYear) {
    console.log(`${targetYear}年の組合員数推計を開始...`);
    
    const baseShareRates = this.calculateBaseShareRates();
    const targetPopulation = this.loadPopulationData(targetYear);
    ...
    const prefectureCode = '00000'; // 全国のみ
    ...
    allAgeGroups.forEach(ageGroup => {
      ...
      // 中高年層：繰り上がり + 減衰
        ...
        if (previousAgeGroup) {
          ...
          const decayRate = this.calculateDecayRate(previousAgeGroup, ageGroup, 2025, targetYear);
  ...
  generateAllProjections() {
    ...
    futureYears.forEach(year => {
      try {
        ...
        // ファイル保存
        const filename = `coop_members_national_${year}.json`;
        const filepath = path.join(this.coopMemberDir, filename);
        fs.writeFileSync(filepath, JSON.stringify(projectionData, null, 2));
        
        console.log(`✓ ${year}年のデータを保存: ${filename}`);
      } catch (error) {
        console.error(`✗ ${year}年の推計でエラー:`, error.message);
      }
    });
  }
}
...
if (require.main === module) {
  const generator = new CoopMemberProjectionGenerator();
  generator.generateAllProjections();
  console.log('組合員数将来推計データの生成が完了しました');
}

module.exports = CoopMemberProjectionGenerator;